...
def gaze_data_callback(gaze_data):
    ...
    canvas.create_oval(
        ((left_x - 5) + (right_x - 5))/2,
        (((left_y - 5) + (right_y - 5))/2),
        (((left_x + 5) + (right_x + 5))/2),
        (((left_y + 5) + (right_y + 5))/2),
        outline="#f15", fill="#1f1", width=5
    )

    screen_width, screen_height = 1920, 1080
    center_x = ((gaze_data['left_gaze_point_on_display_area'][0] + gaze_data['right_gaze_point_on_display_area'][
        0]) / 2) * screen_width
    center_y = ((gaze_data['left_gaze_point_on_display_area'][1] + gaze_data['right_gaze_point_on_display_area'][
        1]) / 2) * screen_height
